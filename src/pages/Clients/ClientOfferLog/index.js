...
const ClientOfferLog = (props) => {
  ...
  const getAllClientLogs = async () => {
    const clientLogsResponse = await userRequest.get(
      `/client-log/getAllLogs?client_id=${client_id}`,
      {
        params: {
    ...
    console.log(clientLogsResponse?.data?.clientsLogs)
    ...
    if (tagExists) {
      return (
        <div className="property-box-footer-left-icon-small">
          <img alt='icon' src={icon} style={{ marginRight: "5px", width: "25px" }} />
        </div>
      );
    } else {
      return null;
        ...
        <table className="table" >
          <thead>
            <tr>
              {columns?.map((iteam, index) => (
                <th key={index} scope="col" style={{ cursor: "pointer", padding: '0px 10px !important' }}>
                  {iteam.name}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {item?.map((iteam, index) => {
              if(iteam.savedSearch === false || iteam.savedSearch === undefined) {
                let propertyIds = [];
                propertyIds.push(iteam.property_id)
                  ...
                  <React.Fragment key={`${iteam._id}-${index}`}>
                    {propertyIdsData.map((propertyId, idx) => (
                      <tr key={`${iteam._id}-${index}-${idx}`}>
                        ...
                        <td>{iteam.booked === true ?  <TiTickOutline /> : "-"}</td>
                        <td
                          style={{
                            color: "#036ae1",
                            textDecoration: "underline",
                        ...
                        <td className="cst-cursor">
                          {iteam.brochure === true ?  <TiTickOutline /> : "-"}
                        </td>
                        <td
              ...
              <div className="modal-body" style={{ marginTop: "-20px" }}>
                ...
                >
                  <div className="col-md-3 px-1">
                    <label className="saved-search-header-title">
                      Nick Name
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="col-md-12 col-12">
                    {loading ? (
                      ...
                      filterClientLogs
                        ...
                        .map((item, i) => {
                          console.log(item?.clientLogs.filter(item => item.savedSearch === false || item.savedSearch === undefined).length)
                          if(item?.clientLogs.filter(item => item.savedSearch === false || item.savedSearch === undefined).length) {
                            ...
                            return (
                              <div key={i}>
                                ...
                                <div className="col-md-8 d-flex justify-content-between travel-section mt-4 px-2">
                                  ...
                                  <div className="travel-budget">
                                    ...
                                    <span className="title-value">
                                      ...
                                      {/* {item?.clientLogs?.[0]?.PriceFilter?.substring(2, item?.clientLogs?.[0]?.PriceFilter?.length - 2)} */}
                                    </span>
                                  </div>
                                ...
                                <div>{tableData(item.clientLogs)}</div>
                              </div>
                            );
                          }
